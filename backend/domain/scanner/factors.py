...
def f2_dividend(b):
    """股息率 = 近12个月每股税前派息合计 / 最新价（东财分红送配）"""
    price = _num((b.get("snapshot") or {}).get("price"))
    divs = b.get("dividends") or []
    if price is None or price <= 0 or not divs:
        return None, None, "缺价格或分红数据"
    from datetime import date, timedelta
    cutoff = (date.today() - timedelta(days=370)).isoformat()
    total = sum((d.get("pretax_bonus_10") or 0) / 10.0 for d in divs
                if (d.get("ex_date") or "") >= cutoff)
    if total <= 0:
        return None, None, "近12个月无分红"
    y = total / price
    score = _lin(y, 0.01, 0.035, 0.0, 100.0)
    return score, round(y * 100, 2), f"股息率={y*100:.2f}%"
...
def f22_northbound(b):
    """北向（沪深股通）持股比例变动（季度披露，东财）"""
    nb = b.get("northbound")
    if not nb:
        return None, None, "北向持股数据缺失"
    change = _num(nb.get("change_rate"))
    if change is None:
        return None, None, "北向持股变动率缺失"
    score = _lin(change, 0.0, 0.8, 0.0, 100.0)
    return score, round(change, 2), f"北向持股比例变动={change:.2f}%（{nb.get('trade_date')}）"


def f23_top_inst(b):
    """机构龙虎榜：近10个交易日机构专用席位净买入额"""
    net = b.get("inst_net")
    if net is None:
        return None, None, "近10日无机构龙虎榜记录"
    if net > 0:
        score = 100.0
    elif net == 0:
        score = 50.0
        ...
        score = 20.0
    return score, round(net / 1e4, 1), f"机构席位净买入 {net/1e4:.0f}万元"


def f24_big_order_ratio(b):
    """大单/主力净买入占比 = 主力净流入 / 当日成交额"""
    snap = b.get("snapshot") or {}
    net = _num(snap.get("main_net_inflow"))   # 元
    amount = _num(snap.get("amount"))         # 元
    if net is None or amount in (None, 0):
        return None, None, "缺主力资金流或成交额"
    ratio = net / amount
    score = _lin(ratio, 0.0, 0.08, 0.0, 100.0)
    return score, round(ratio * 100, 2), f"主力净流入占成交额={ratio*100:.2f}%"
...
def f26_holder_change(b):
    ...
    if not holders:
        return None, None, "股东户数数据缺失"
    chg = _num(holders[0].get("ratio"))
    if chg is None:
        return None, None, "股东户数变动率缺失"
    if chg <= -5:
        score = 100.0
    elif chg >= 5:
        score = 0.0
    else:
        score = _lin(chg, 5.0, -5.0, 0.0, 100.0)
    return score, round(chg, 2), f"股东户数环比{chg:+.1f}%"


def f27_margin_ratio(b):
    """融券余额/融资余额 历史分位（做空力量枯竭=低分位=满分）"""
    margin = b.get("margin") or []
    ratios = []
    for r in margin:
        rzye = _num(r.get("rzye"))
        rqye = _num(r.get("rqye"))
        if rzye not in (None, 0) and rqye is not None:
            ratios.append(rqye / rzye)
    if len(ratios) < 5:
        return None, None, "融资融券数据不足"
    cur = ratios[0]  # margin 降序，最新在前
    rank = sum(1 for x in ratios if x <= cur) / len(ratios)
    if rank <= 0.2:
        score = 100.0
    elif rank >= 0.8:
        score = 0.0
    else:
        score = _lin(rank, 0.2, 0.8, 100.0, 0.0)
    return score, round(cur, 4), f"融券/融资={cur:.4f}（历史{rank*100:.0f}%分位）"
...
DIM_NAMES = {
    ...
    "chip": "情绪", "safety": "安全边际", "macro": "景气与宏观",
}

# 维度内因子权重（每个维度内合计 = 1.0，按「数据可靠性 + 因子区分度」设定）
# 情绪维度（chip）中「综合情绪因子 g26」权重最高（情绪占比放大）
FACTOR_WEIGHTS = {
    # 价值估值（维度权重 13%）
    "f1": 0.30, "f2": 0.20, "f3": 0.25, "f4": 0.25,
    # 资产质量（17%）
    "f5": 0.30, "f6": 0.25, "f7": 0.25, "f8": 0.20,
    # 盈利成长（16%）
    "f9": 0.30, "f10": 0.25, "f11": 0.25, "f12": 0.20,
    # 趋势技术面（12%）——均线/趋势 + MACD/DMI/TRIX/DMA/EMA
    "f13": 0.15, "f14": 0.12, "f15": 0.08, "f16": 0.10,
    "g1": 0.15, "g2": 0.10, "g3": 0.08, "g4": 0.07, "g5": 0.15,
    # 量价动能（11%）——量价 + 超买超卖/量能
    "f17": 0.12, "f18": 0.08, "f19": 0.10, "f20": 0.12,
    "g6": 0.07, "g7": 0.06, "g8": 0.06, "g9": 0.07, "g10": 0.05,
    "g11": 0.06, "g12": 0.05, "g13": 0.06, "g14": 0.10,
    # 主力资金流（11%）
    "f21": 0.30, "f22": 0.20, "f23": 0.25, "f24": 0.25,
    # 情绪（12%）——筹码分布 + 综合情绪因子（g26 权重最高）
    "f26": 0.10, "f27": 0.08,
    "g20": 0.10, "g21": 0.10, "g22": 0.07, "g23": 0.06,
    "g24": 0.06, "g25": 0.08, "g26": 0.35,
    # 安全边际（7%）——波动/风险收益
    "f28": 0.18, "f29": 0.18, "f30": 0.18,
    "g15": 0.10, "g16": 0.10, "g17": 0.10, "g18": 0.08, "g19": 0.08,
    # 景气与宏观（5%）
    "f31": 0.55, "f33": 0.45,
}


# 扩展因子（细化技术指标 + 筹码 + 情绪），由 factors_extra.py 提供
from .factors_extra import EXTRA_FACTORS  # noqa: E402
